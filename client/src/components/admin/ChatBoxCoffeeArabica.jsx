...
import { createChatCoffeeArabica, getChatCoffeeArabica, getLastMessageChatCoffeeArabica, getRooms } from "../../services/api";
import { createCommandCoffeeArabica, getCommandCoffeeArabica } from "../../services/api_command_coffeearabica";
const ChatBoxCoffeeArabica = () => {
    ...
    const handleSendMessage = async () => {
        if (message.trim() !== '') {
            ...
            const response = await createChatCoffeeArabica(newMessage);
    ...
    const fetChatCoffeeVn = async () => {
        try {
            const chatList = await getChatCoffeeArabica();
    ...
    const fetCommandCoffeeVn = async () => {
        try {
            const commandVn = await getCommandCoffeeArabica();
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const response = await getLastMessageChatCoffeeArabica();
    ...
    const handleCreateCommandCoffeeVn = async (value, money, username) => {
        ...
        const response = await createCommandCoffeeArabica(newCommand);
        setShowPopup(false);
    ...
    return (
        <div className='flex flex-col w-full h-auto '>
            ...
            <div className="flex justify-start mt-5 ml-5 md:justify-center sm:justify-center lg:justify-start">
                <p className="font-bold">Phòng chat Coffee Arabica</p>
            </div>
...
export default ChatBoxCoffeeArabica